...
import com.idevicesinc.sweetblue.BleServer.OutgoingListener;
import com.idevicesinc.sweetblue.BleServer.ServiceAddListener;
import com.idevicesinc.sweetblue.BleServer.StateListener;
...
public class MyActivity extends Activity
{
    ...
    // The instance of BleServer we will be using to server the time
    ...
    private IncomingListener.Please onIncomingEvent(IncomingListener.IncomingEvent incomingEvent)
    {
        ...
        else if(incomingEvent.target() == BleServer.ExchangeListener.Target.DESCRIPTOR)
        {
    ...
    private void onStateEvent(StateListener.StateEvent stateEvent)
    {
    ...
    private void onServiceAdd(ServiceAddListener.ServiceAddEvent serviceAddEvent)
    {
    ...
    private void onDiscoverEvent(DiscoveryListener.DiscoveryEvent discoveryEvent)
    {
        ...
        {
            discoveryEvent.device().connect(stateEvent ->
            {
                if(stateEvent.didEnter(BleDeviceState.INITIALIZED))
                {
                    ...
                    m_bleServer.connect(stateEvent.device().getMacAddress());
                }
            });
        }
    }
